...
var PREFIX_IRI = "http://mitatac.demo.marklogic.com";
...
var NS_EPI = "http://mitatac.demo.marklogic.com/predicates#";
...
function loadTaxonomy(context, params, input) {
  ...
  if (extension !== 'xml' && extension !== 'rdf' && extension !== 'ttl') {
    ...
    if (extension === 'xml' || extension === 'rdf') {
      triples = sem.rdfParse(fn.head(xdmp.unquote(content)).root, [baseURI, 'rdfxml']);
    } else {
...
function listSynonyms(name, taxonomyId) {
  ...
  //   sem.iri("http://mitatac.demo.marklogic.com/taxonomy/dmlc")
  // )
...
function listSuggestsOld(name, taxonomyId) {
  var suggests = [];
...
function doSaveNamespace(map) {
  var taxonomyId = map["id"];
  var namespace = map["namespace"];
  console.log("doSaveNamespace: " + JSON.stringify(map));
  sem.rdfInsert(
...
function processTriples(file, extension, taxonomyId, progressId) {
  ...
  if (extension === "xml" || extension === "rdf") {
    ...
    triples = sem.rdfParse(fn.head(xdmp.unquote(file)).root, [baseURI, "rdfxml"]);
  } else {
...
function createSynonyms(taxonomyId, progressId) {
  ...
  saveThesaurusDoc(thesaurusUri, fn.head(thesaurusDoc).root);
