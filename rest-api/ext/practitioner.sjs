...
function get(context, params) {
  ...
  if (params["_id"]) {
    ...
    xdmp.log("Getting Practitioner by _id: " + params["_id"]);
  } else if (params["practitionerId"]) {
    xdmp.log("Getting Practitioner record: " + params["practitionerId"]);
    response = p.getPractitionerById(params["practitionerId"], false);
  } else {
    xdmp.log("getting Pracitioners" || params["_count"]);
    response = p.getPractitioners(params["_count"] * 1);
  };
